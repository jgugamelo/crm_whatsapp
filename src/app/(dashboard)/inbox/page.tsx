...
export default function InboxPage() {
  ...
  const handleConversationsLoaded = useCallback(
    (loaded: Conversation[]) => {
      ...
      // If user ALREADY has an active conversation open, update its fields in-place without changing thread!
      if (activeConversationRef.current) {
        const freshActive = loaded.find((c) => c.id === activeConversationRef.current?.id);
        if (freshActive) {
          setActiveConversation((prev) => (prev ? { ...prev, ...freshActive } : freshActive));
          if (freshActive.contact) {
            setActiveContact(freshActive.contact);
          }
        }
        return;
      }

      // If NO conversation is open yet, resolve initial deep-link ?c=<id>
      if (
        deepLinkConvId &&
